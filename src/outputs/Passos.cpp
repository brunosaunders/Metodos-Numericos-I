...
void Passos::exibir_passos_raiz(NewtonRaphson* metodo, int index) {
    ...
    std::cout << "|";
    Tabela::formata_palavra("k", largura_padrao);
    Tabela::formata_palavra("xk", largura_padrao);
    Tabela::formata_palavra("xk-1", largura_padrao);
    Tabela::formata_palavra("f(xk)", largura_padrao);
    Tabela::formata_palavra("f'(xk)", largura_padrao);
    Tabela::formata_palavra("e1", largura_padrao);
    Tabela::formata_palavra("e2", largura_padrao);
    std::cout << std::endl;
    Tabela::exibir_tampa_tabela(largura_total_tabela);
    ...
    for (int k = 1; k < metodo->get_iteracoes_de_x(index).size(); k++) {
        ...
        Tabela::formata_numero_cientifico(f_derivada_xk, precisao_fx_xk, largura_padrao);
        Tabela::formata_numero_cientifico(e1, precisao_fx_xk, largura_padrao);
        Tabela::formata_numero_cientifico(e2, precisao_fx_xk, largura_padrao);
        std::cout << std::endl;
