...
void QuadroComparativo::exibir_quadro(std::vector<NewtonRaphson*> metodos) {
    ...
    Tabela::exibir_tampa_tabela(largura_total_tabela);

    std::cout << "|";
    Tabela::formata_palavra("k", largura_k);
    Tabela::formata_palavra("Método", largura_padrao);
    Tabela::formata_palavra("a3", largura_padrao);
    Tabela::formata_palavra("a2", largura_padrao);
    Tabela::formata_palavra("lambda", largura_padrao);
    Tabela::formata_palavra("r1", largura_padrao);
    Tabela::formata_palavra("r2", largura_padrao);
    Tabela::formata_palavra("r3", largura_padrao);
    Tabela::formata_palavra("iter-total", largura_padrao);
    Tabela::formata_palavra("iter-média", largura_padrao);
    Tabela::formata_palavra("quebra?", largura_padrao);
    std::cout << std::endl;
    Tabela::exibir_tampa_tabela(largura_total_tabela);
    ...
    for (auto& metodo : metodos) {
        ...
        for (int i=0; i < 3; i++) {
            if (i < raizes.size()) {
                Tabela::formata_numero_cientifico(raizes[i], precisao_max, largura_padrao);
            } else {
                Tabela::formata_palavra("null", largura_padrao);
            }
        } 
